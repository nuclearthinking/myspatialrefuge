...
local function startCorpseSearch(deathX, deathY, deathZ, username, playerOnlineId, diedInRefuge, earnedXp)
    ...
    LOG.info("Starting corpse search for %s at %d,%d,%d (onlineId=%s inRefuge=%s earnedXpEntries=%d total=%.3f)",
        tostring(username), deathX, deathY, deathZ, tostring(playerOnlineId), tostring(diedInRefuge),
        earnedXpCount, earnedXpTotal)
    ...
    local function findCorpse()
        ...
        if tickCount > maxTicks then
            ...
            LOG.info("Timeout finding corpse of %s after %d ticks", tostring(username), tickCount)
            return
        end
        ...
        if corpse then
            Events.OnTick.Remove(findCorpse)
            LOG.info("Found corpse for %s at tick=%d corpseOnlineId=%s",
                tostring(username), tickCount,
                tostring(corpse.getCharacterOnlineID and corpse:getCharacterOnlineID() or "n/a"))
