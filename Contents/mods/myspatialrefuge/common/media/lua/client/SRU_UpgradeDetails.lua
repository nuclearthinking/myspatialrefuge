...
function SRU_UpgradeDetails:render()
    ...
    if currentLevel >= maxLevel then
        levelText = getTextFormatted("UI_RefugeUpgrade_LevelMax", currentLevel, maxLevel)
            or string.format("Level %d/%d (MAX)", currentLevel, maxLevel)
    else
        levelText = getTextFormatted("UI_RefugeUpgrade_LevelNext", currentLevel, maxLevel, self.level or currentLevel + 1)
            or string.format("Level %d/%d -> %d", currentLevel, maxLevel, self.level or currentLevel + 1)
    end
...
function SRU_UpgradeDetails:formatEffect(name, value)
    ...
    if self:isAbstractEffect(name) then
        ...
        local levelText = getTextFormatted("UI_Effect_LevelFormat", level) or string.format("level %d", level)
